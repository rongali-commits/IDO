...
export function EssayCard({ essay, featured = false }: { essay: Essay; featured?: boolean }) {
  return (
    <article className={`essay-card ${featured ? "essay-card-featured" : ""}`}>
      <Link href={`/essays/${essay.slug}`} className="card-cover-link" tabIndex={-1} aria-hidden="true"><EssayCover accent={essay.accent} src={essay.coverImage} alt={essay.coverAlt} compact={!featured} priority={featured} decorative /></Link>
