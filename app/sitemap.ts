...
export default function sitemap(): MetadataRoute.Sitemap {
  const pages = ["", "/essays", "/topics", siteConfig.authorPath, "/work-with-me", "/newsletter", "/reading-list", "/now", "/editorial-policy", "/privacy"].map((path) => ({ url: `${siteConfig.url}${path}`, lastModified: new Date() }));
