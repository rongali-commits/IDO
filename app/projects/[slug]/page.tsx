...
import { designStudies } from "@/lib/design-studies";
...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  const study = designStudies[slug];
  if (!detail) notFound();
  ...
  return (
    <main>
      ...
      {study && <section className="case-design shell" aria-labelledby="design-title"><div className="case-design-heading"><div><p className="section-kicker">The design thinking</p><h2 id="design-title">A point of view,<br />in every detail.</h2></div><p>{study.premise}</p></div><div className="design-decision-grid">{study.decisions.map((decision, i) => <article key={decision.title}><span>0{i+1} / Design decision</span><h3>{decision.title}</h3><p>{decision.detail}</p></article>)}</div></section>}
      <section className="case-sections shell">
        {product.walkthroughVideo && <section className="case-walkthrough" id="walkthrough" aria-labelledby="walkthrough-title"><div className="case-walkthrough-heading"><div><p className="section-kicker">Product walkthrough</p><h2 id="walkthrough-title">See the complete<br /><em>decision workflow.</em></h2></div><p>{product.walkthroughDescription ?? "A narrated tour from customer evidence to themes, prioritized opportunities, cited AI answers, and stakeholder-ready reports."}</p></div><video controls playsInline preload="metadata" poster={product.image} aria-label={`${product.name} narrated product walkthrough`}><source src={product.walkthroughVideo} type="video/mp4" />Your browser does not support the video player.</video><p className="case-video-note">{product.walkthroughMeta ?? "Full HD · 1 minute · Voice-over included"}</p></section>}
        ...
        {(product.sourceKitUrl || product.contraUrl || product.offerNote) && <section className="case-purchase shell" aria-labelledby="purchase-title"><p className="section-kicker">Make it your own</p><h2 id="purchase-title">A foundation for your next build.</h2><div className="case-actions">{product.sourceKitUrl && <a className="button button-dark" href={product.sourceKitUrl} target="_blank" rel="noreferrer">{product.sourceKitLabel ?? "Source kit"} · {product.sourceKitPrice} <span aria-hidden="true">↗</span></a>}{product.contraUrl && <a className="button button-outline" href={product.contraUrl} target="_blank" rel="noreferrer">Setup service · from {product.startingPrice} <span aria-hidden="true">↗</span></a>}</div><p className="case-offer-note">{product.offerNote ?? "Source kit: downloadable code and documentation. Setup service: a separately scoped implementation. Hosting, domains, paid APIs, and extra integrations are not included in the source-kit price."}</p></section>}
