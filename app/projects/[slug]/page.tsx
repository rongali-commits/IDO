  ...
  return (
    <main>
      ...
      <header className="case-hero shell">
        ...
        <div className="case-hero-grid"><h1>{product.name}</h1><div className="case-intro"><p className="section-kicker">{product.stage} · Independent Noerong product</p><p>{product.summary}</p><div className="case-actions"><a className="button button-dark" href={product.liveUrl} target="_blank" rel="noreferrer">{product.stage === "Working demo" ? "Explore working demo" : "View live product"} <span aria-hidden="true">↗</span></a>{product.sourceKitUrl && <a className="button button-outline" href={product.sourceKitUrl} target="_blank" rel="noreferrer">Source kit · {product.sourceKitPrice} <span aria-hidden="true">↗</span></a>}{product.contraUrl && <a className="button button-outline" href={product.contraUrl} target="_blank" rel="noreferrer">Setup service · from {product.startingPrice} <span aria-hidden="true">↗</span></a>}</div>{product.sourceKitUrl && <p className="case-offer-note">Source kit: downloadable code and documentation. Setup service: a separately scoped implementation. Hosting, domains, paid APIs, and extra integrations are not included in the source-kit price.</p>}</div></div>
      </header>
